...
|---|---|---|
| **`goal`** (`tickets` → `spec` → `plan` → run → `next`) | Source (Jira / GitHub issue / spec file / note) → a grilled plan whose every slice declares in advance the command that proves it, the files it may touch and its diff budget → execution in one of **two modes**: `manual`, where you review each slice and a checkpoint replays its acceptance commands, or `commit+pr`, where a **program** judges each slice by exit code and is the only thing that commits | The requirement is fuzzy **or** you want it delivered across sessions with an audit trail. Pick `manual` to stay in the loop slice by slice, `commit+pr` to hand over the whole feature. This is the full pipeline, and the only one with a machine verdict. |
...
| **`goal:grill-adversarial`** | A **front / interactive** feature with many states — enumerates the `(state × action)` space and invariants before iterations freeze. Runs inside `goal:spec`, or standalone. |
...
├─ A chantier — several independent outcomes, no single success signal
│     → goal:tickets    (ordered backlog, elaborated just-in-time; then one ticket at a time below)
...
- **`goal` subsumes `/spec-first-dev`'s grilling.** `goal:spec` + `goal:plan`'s Session-1 phases (understand → grill → specify) do the same spec-building work, then add autonomous execution. Use `/spec-first-dev` **only** when you want that conversation without `goal`'s branch / iteration / log machinery.
