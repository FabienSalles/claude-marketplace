...
description: "ACTIVATE when adding or modifying frontend behaviour in a Symfony project — any new feature whose Twig template would otherwise contain inline script or style blocks, or any new JS/CSS file to load for a specific page. ACTIVATE for 'inline script in twig', 'inline style in twig', 'WebpackEncore', 'addEntry', 'encore_entry_link_tags', 'encore_entry_script_tags', 'where to put my JS', 'how to load CSS for one page', 'FormType injecting class', 'js- hook in template'. Covers: forbidden inline script/style tags in Twig, one Encore entry per feature, per-page asset loading via stylesheets/javascripts blocks extending parent, SCSS imported by the JS module (one entry bundles both), FormType stays domain/server (no CSS-only classes via `attr`), `js-*` hooks added at render time. DO NOT use for: jQuery style (see jquery:jquery), Twig conventions (see symfony:twig-conventions), FormType conventions (see symfony:symfony-form), autonomous visual debugging (see frontend:frontend-best-practices)."
---
