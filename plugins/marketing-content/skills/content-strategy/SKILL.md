...
For cadence recommendations, batching workflows, and calendar production, delegate to the **`content-calendar`** skill. This skill stops at "what to publish and why"; the calendar owns "when to publish and in what batches".
...
For repurposing playbooks (blog → thread → carousel → video → newsletter), delegate to the **`content-repurposing`** skill.
...
For the actual month-by-month plan (dates, batching, owner assignments), invoke **`content-calendar`** with the topic-cluster map produced in section 2.
