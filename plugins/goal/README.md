...
| `goal-run.ts` + `run/*.ts` | `scripts/` + `src/run/` | The runner: 1,618 lines, the entry point plus 14 modules (preflight, sweep, lock, iteration, publish, close, report) |
...
| `tests/run.sh` | `tests/` | 415 tests across 47 files. Wraps `node --test` and additionally refuses a zero-pass run, an undeclared skip, and a missing summary (a bare `node --test` exits 0 on a glob matching nothing) |
