...
| `goal-auto.js` | `workflows/goal-auto.js` | The orchestration layer `/goal:auto` launches by path: survey, then one implementer and one gate per iteration, in plan order. Returns `{status, plan, iteration, detail, landed, notAttempted}` and stops at the first refusal. It has no disk and no shell, so it cannot commit even by mistake. |
