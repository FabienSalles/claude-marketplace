...
import { PAUSED, pendingInNode, repo, run } from './support/goal-run-harness.ts';
...
test('a quota-shaped failure sleeps and relaunches the same iteration until it lands', { skip: pendingInNode }, () => {
  const fixture = repo();
...
test('a quota that never reopens pauses the run after a bounded number of relaunches, not forever', { skip: pendingInNode }, () => {
  const fixture = repo();
...
test('a failure with no quota shape pauses immediately, without a retry', { skip: pendingInNode }, () => {
  const fixture = repo();
...
test("the implementer's brief warns the tree may already hold an interrupted attempt", { skip: pendingInNode }, () => {
