...
- **Reason for rejection**: maintainability, before any defect. 594 lines of shell in one file
  ...
  converged on the same move: serious harnesses live in typed, tooled languages. Claude Code is
  in TypeScript, Goose in Rust, and OpenAI rewrote Codex CLI from TypeScript to native Rust when
